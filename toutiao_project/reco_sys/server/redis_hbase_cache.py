...
def get_cache_from_redis_hbase(temp, hbu):
    """
    进行用户频道推荐缓存结果的读取，redis充当了wait_recommend表的功能
    :param temp: 用户请求的参数
    :param hbu: hbase工具
    :return:
    """
    ...
    if res:
        ...
        # 删除redis这个键，用户拿过后，我们就删除了，所以删除对应的键，我们是边取，边删的
        ...
        if len(hbase_cache) > 100:
            ...
            cache_client.zadd(key, list(zip(range(len(cache_redis),cache_redis))))
            # 剩下的放回wait hbase结果
            ...
            # 清空wait_recommend，放一个空列表
            hbu.get_table_put('wait_recommend',
                              'reco:{}'.format(temp.user_id).encode(),
                              'channel:{}'.format(temp.channel_id).encode(),
                              str([]).encode())

            # 放入redis缓存,应该改这里，改成了上面
            cache_client.zadd(key,5,'100')
            # cache_client.zadd(key, [[32, "100"], [64, "200"]])
            # cache_client.zadd(key, list(zip(hbase_cache, range(len(hbase_cache)))))
